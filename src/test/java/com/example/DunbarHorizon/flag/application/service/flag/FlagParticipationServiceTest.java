...
class FlagParticipationServiceTest {
    ...
    @Test
    @DisplayName("Manager가 반환한 참여자를 저장한다")
    void participateInFlag_SavesParticipantFromPolicy() {
        // given
        FlagParticipant mockParticipant = mock(FlagParticipant.class);
        given(flagParticipationManager.participate(FLAG_ID, USER_ID)).willReturn(mockParticipant);
    ...
    @Test
    @DisplayName("Manager에서 FlagNotFoundException이 발생하면 서비스에서 그대로 전파된다")
    void participateInFlag_PolicyThrowsFlagNotFoundException_Propagates() {
        // given
        willThrow(new FlagNotFoundException(FLAG_ID))
                .given(flagParticipationManager).participate(FLAG_ID, USER_ID);
    ...
    @Test
    @DisplayName("Manager에서 FlagParticipationDuplicateException이 발생하면 서비스에서 그대로 전파된다")
    ...
    @Test
    @DisplayName("Manager가 반환한 FlagParticipant를 삭제한다")
    void leaveFlag_DeletesParticipantFromPolicy() {
        // given
        FlagParticipant mockParticipant = mock(FlagParticipant.class);
        given(flagParticipationManager.unparticipate(FLAG_ID, USER_ID)).willReturn(mockParticipant);
