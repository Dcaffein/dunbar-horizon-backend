...
public class Cast {
    ...
    private List<CastReply> replies = new ArrayList<>();
    ...
    @Builder
    public Cast(Long creatorId, String creatorNickname, String creatorProfileImageUrl,
    ...
    public void markAsRead(Long userId) {
        if (!isRecipient(userId)) {
            throw new CastAccessDeniedException("해당 캐스트의 수신자가 아닙니다.");
        }
    ...
    public CastReply createReply(Long replierId, String nickname, String profileImageUrl,
                                 String text, List<String> imageUrls, boolean isPublic) {
        if (isExpired()) {
            throw new CastInvalidStateException("만료된 캐스트에는 답장할 수 없습니다.");
        }
        if (!isRecipient(replierId)) {
            throw new CastAccessDeniedException("이 캐스트에 답장을 남길 권한이 없습니다.");
        }

        markAsRead(replierId);

        return CastReply.builder()
    ...
    public void updateReply(Long requesterId, String replyId, String newText, List<String> newImageUrls) {
        if (isExpired()) {
            throw new CastInvalidStateException("만료된 캐스트의 답장은 수정할 수 없습니다.");
        }

        CastReply target = findReplies(replyId);

        if (!target.getReplierId().equals(requesterId)) {
            throw new CastAccessDeniedException("답장 수정 권한이 없습니다.");
        }
    ...
    public void validateReplyDeletion(Long requesterId, String replyId) {
        ...
        if (!target.getReplierId().equals(requesterId) && !this.creatorId.equals(requesterId)) {
            throw new CastAccessDeniedException("답장 삭제 권한이 없습니다.");
        }
    }

    private CastReply findReplies(String replyId) {
        return replies.stream()
                .filter(r -> r.getReplyId().equals(replyId))
                .findFirst()
                .orElseThrow(() -> new CastInvalidStateException("존재하지 않는 답장입니다."));
    }

    private void validateRecipientIds(List<Long> recipientIds) {
        if (recipientIds == null || recipientIds.isEmpty()) {
            throw new CastInvalidStateException("수신자가 최소 한 명 이상 지정되어야 합니다.");
        }
    }
}